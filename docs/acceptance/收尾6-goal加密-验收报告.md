...
## 10. 补丁（2026-09-24）：脱敏占位符的「N 字」报错了

条件验收通过后，用户从第 9.1 节贴的**真实返回样本**里看出了一个 bug：

| 被替换的原文 | 实际长度 | 报出来的字数 | |
| --- | --- | --- | --- |
| `密码Zx9!secret` | 10 | `·10字` | ✅ |
| `银行卡6222021234567890` | 16 | `·79字` | ❌ |
| `身份证110101199003071234` | 18 | `·79字` | ❌ |

第一个对、后两个都变成 79 —— 用户给的两个怀疑方向：①「后一次替换的长度用了已被前一次改写过的字符串」
②「card/ID 正则贪婪吞掉了过长片段」。

### 10.1 根因：①方向对，②不是

**不是贪婪。** 探针把 `String.replace` 回调收到的实参逐个打出来（`typeof` 一起打）：

```
[card 回调实参] 个数 = 3
  arg[0] (string) = "6222021234567890"        ← 真正被替换掉的那段（16 字）
  arg[1] (number) = 3                          ← offset
  arg[2] (string) = "卡号 6222021234567890 结束" ← **整个输入串**

[password 回调实参] 个数 = 5
  arg[0] (string) = "密码Zx9!secret"
  arg[1] (string) = "密码"                      ← 前缀组
  arg[2] (string) = "Zx9!secret"               ← 值组（10 字）
  arg[3] (number) = 4                          ← offset
  arg[4] (string) = "登录用 密码Zx9!secret，实名" ← 整个输入串
```

`replace` 回调的实参形状**随正则有没有捕获组而变**：有 n 组 → `(match, g1…gn, offset, string)`；
没有组 → `(match, offset, string)`。而 `VALUE_PATTERNS` 里五条正则形状不一致：

- `password` / `otp` / `cvv` 带 **2 个捕获组** → 旧代码取 `m[2]` 正好是值 → 字数对；
- `card` / `idcard` 用的是 `(?:…)` **非捕获**组 → 旧代码取 `m[1]` 是 offset（数字，被
  `typeof === 'string'` 挡掉 → 前缀变空），取 `m[2]` 拿到的是**整个输入串** → 字数 = 整句长度。

那个 79 正是「原文 71 字，被 password 那一轮替换成 79 字之后」的整串长度 —— 所以用户的方向①
（「用了已被前一次改写过的字符串」）是对的，只是机制不是贪婪匹配，而是**长度取错了来源**。
正则本身一个字符都没多吞（第 10.4 段的反证 R2 专门验这件事）。

旧代码（错的那三行）：

```ts
const prefix = typeof groups[1] === 'string' ? groups[1] : '';
const value  = typeof groups[2] === 'string' ? groups[2] : whole;  // ← 无捕获组时这是整个输入串
const len = [...value].length;
```

### 10.2 修法

按实参形状还原捕获组个数，无组时用 `match` 本身当「被替换掉的那段」（`redactForStorage`）：
...
修完的实际输出（真库 `GET /agent/task/current`）：

```json
{"id":1,
 "displayTitle":"帮我查一下 银行卡[已脱敏·card·16字] 的余额，登录用 密码[已脱敏·password·10字]，实名 身份证[已脱敏·card·18字]",
 "goal":"帮我查一下 银行卡6222021234567890 的余额，登录用 密码Zx9!secret，实名 身份证110101199003071234"}
```

**影响面**：只是占位符里那个数字变了（`·79字` → `·16字`），敏感值本来就被替换掉了 —— 所以这
**不是泄漏**，是「给人看的信息错了」（运维看到 `·79字` 会以为涂掉了一大段）。
调用方（`channels.ts` 三处、`workers.ts` 两处、`agent.ts` 的 `scrubStepSummary` / `taskDisplayTitle`）
都不依赖那个数字，全仓也没有任何测试硬编码过占位符字数（grep 过 `已脱敏·`）。

### 10.3 用户要求补的三组断言（`scripts/verify/redact-placeholder.mts`，新增）

★ 一律用**整串精确相等**，不用 `includes`：`includes('已脱敏')` 这类断言在字数错成 79 时照样全绿 ——
那正是这个 bug 能溜过上一轮验收的原因（第 9.1 节那些断言只查了「非空 / 不含敏感词 / ≤80 字」）。

**① 占位符字数 == 被替换原文的实际长度**（12 条，每条三连断言：整串相等 + 原值一个不剩 + 字数逐个核对）

```
password 中文   登录用 密码Zx9!secret，别的不该动  → 登录用 密码[已脱敏·password·10字]，别的不该动
password 英文   pwd: hunter2secret end            → pwd: [已脱敏·password·13字] end
otp             发送验证码 839201 给用户           → 发送验证码 [已脱敏·otp·6字] 给用户
card 16 位      卡号 6222021234567890 结束         → 卡号 [已脱敏·card·16字] 结束
card 13 位下限  订单 1234567890123 号              → 订单 [已脱敏·card·13字] 号
card 19 位 / 20 位上限                             → ·19字 / ·20字
card 空格分组   6222 0212 3456 7890                → ·19字（16 位数字 + 3 个空格：分隔符也被替换掉了，算进去才如实）
card 横线分组   6222-0212-3456-7890                → ·19字
idcard 末位 X   11010119900307888X                 → [已脱敏·idcard·18字]
idcard 18 位    110101199003071234                 → [已脱敏·card·18字]（card 那条先命中，见 10.5-2）
cvv             CVV 739 校验通过                   → CVV [已脱敏·card·3字] 校验通过
纯卡号无上下文  6222021234567890                   → [已脱敏·card·16字]（占位符自身才 14 字 —— 报的是原文长度，不是占位符长度）
```

**② 多个敏感值连排时每个占位符各自正确**

```
用户报的那句原话 → card·16, password·10, card·18   （三个字数互不相同，各自对应自己那段）
回归钉：输出里不许再出现 71 / 79（原文长度 / 替换后整串长度），也不许三个字数全一样
两串数字只隔一个空格 → 卡号 [·16字] 实名 [·18字]（各自成段，不并成一段）
同类别连排两个验证码 → [·6字] 与 [·6字]（第二个不报整串长度）
五类混排（密码/卡号/验证码/身份证/CVV）→ 字数依次 10, 16, 6, 18, 3
```

**③ 反例证明贪婪匹配不会吞掉非敏感内容**

```
余额 12345 元，卡号 6222021234567890，备注 hello world，订单 A-99，第3章
  → 余额 12345 元，卡号 [已脱敏·card·16字]，备注 hello world，订单 A-99，第3章   （两侧逐字节原样）
完全不含敏感值的句子 → 输出与输入逐字节相同，一个占位符都没有
数字紧贴中文 卡号6222021234567890元 → 卡号[·16字]元（中文不被带走）
两串夹中文 …16位 和 …18位 → [·16字] 和 [·18字]（不跨接）
分组分隔符只认**单个**空格/横线：6222 0212␣␣3456 7890（双空格）→ 整句原样
边界钉：12 位（下限外）不动、21 位（上限外）不动、34 位连排不动 ← 后两条是**已知缺口**，见 10.5-1
```

**④ 附带**：`scrubTaskText` 的「」形状与值形状叠加（`写入「[已脱敏·16字]」并把 密码[已脱敏·password·10字] 填进去`）、
`taskDisplayTitle` 的 80 字上限 / 全敏感时非空 / 空串回落「任务」/ 连续空白折叠。

两个 DB 验收脚本里的显示字段断言也一并升级成整串精确相等
（真库 +4 项 → 115 PASS；pglite +2 项 → 56 PASS），这样 bug 复发时在**真链路**上也会红，不只是单测红。

### 10.4 反证：把修复改坏，新断言必须变红

| 变异 | 改法 | `verify:redact` | 连带 | 还原 |
| --- | --- | --- | --- | --- |
| **R1** | 把 `redactForStorage` 退回旧写法（不看实参形状，直接取 `m[1]`/`m[2]`） | **EXIT 1，33 PASS / 24 FAIL** | pglite **EXIT 1，54 PASS / 2 FAIL**（displayTitle 与 steps 两条精确断言） | md5 `6979acba…` 一致 |
| **R2** | 把 card 正则改成**真贪婪** `/\S*(?:\d[ -]?){12,19}\d\S*/g`（连两侧非空白一起吞） | **EXIT 1，49 PASS / 8 FAIL** | ③-1 与 ③-3 红：`，备注` 被吞进占位符、`卡号…元` 整段消失 | md5 `6979acba…` 一致 |

R1 的典型输出（`·22字` 正好等于整句 `卡号 6222021234567890 结束` 的长度，机制坐实）：

```
★FAIL card（16 位，用户报的那条）：整串精确相等（不是 includes）
      实际："卡号 [已脱敏·card·22字] 结束"
      期望："卡号 [已脱敏·card·16字] 结束"
PASS  card（16 位，用户报的那条）：被替换的原文一个字符都不剩     ← 注意这条仍然绿：bug 不泄漏，只是字数错
```

R2 的典型输出（证明 ③ 组不是摆设 —— 真贪婪时它立刻抓到）：

```
★FAIL ③-1：只有 16 位卡号被替换，两侧中文/英文/短数字/标点全部逐字节原样
      实际："余额 12345 元，卡号 [已脱敏·card·19字] hello world，订单 A-99，第3章"
      期望："余额 12345 元，卡号 [已脱敏·card·16字]，备注 hello world，订单 A-99，第3章"
★FAIL ③-3：数字串紧贴中文（无空格）也只吞数字，中文不被带走
      实际："[已脱敏·card·19字]"
      期望："卡号[已脱敏·card·16字]元"
```

### 10.5 这次查到但没修

1. **≥21 位连续数字串整段漏网**（`card` 命中 13~20 位、`idcard` 命中正好 18 位）：
   `流水 123456789012345678901 号`（21 位）与 `卡号 6222021234567890110101199003071234 结束`
   （两个卡号紧贴无分隔 = 34 位）**完全不脱敏**，明文会原样落进 `payload.steps`。
   没在本批修：放宽上限会把 21+ 位的订单号 / 流水号 / 时间戳也涂掉（误伤可读性），
   而 `detectSensitive` 用的是**同一组边界**，要改必须两处一起改 + 重过一遍验收 —— 那是独立的一批活。
   已用 ③-7 / ③-8 两条断言**钉住现状**：谁改边界，这两条会先红，逼他读到这段说明。
2. **18 位纯数字身份证被标成 `card` 而不是 `idcard`**：`card`（13~20 位）排在 `idcard` 前面先命中。
   标签只是给人看的类别名、不含内容，且改顺序会连带改 `detectSensitive` 的判定类别与既有验收输出，
   所以本批不动（末位是 X 的身份证仍正确标 `idcard`）。
3. 沙箱在回合边界把 `node_modules` / `pgverify` / `backups` / `verify-logs` 全清了，
   本地 `.git` 也被重置回基线 `ac75d63`。已用 `git fetch` + `git reset --mixed 0dbbddb`（不碰工作树、
   不用 `--hard`）把分支指针接回远端提交，工作树与推上去的内容逐字节一致；依赖重装、真库验收环境
   （embedded-postgres 17.10 / UTF8 / 55432）按原配方重建。**教训**：证据要当场抄进仓库里的文档，
   留在 `/home/user` 下的日志随时会没。

### 10.6 脚本与 EXIT

| 命令 | 结果 | EXIT |
| --- | --- | --- |
| `npm run verify:redact`（**本轮新增**，已挂进 `verify` 主链最前面） | **57 PASS / 0 FAIL** | **0** |
| `npm run verify` | 全链绿（redact → tools → r4 → orch → websearch → memory → persona → collab → batches → pglite 56/0） | **0** |
| `npm run verify:db` | 收尾1 全 PASS、收尾2 全 PASS、收尾6 真库 **115 PASS / 0 FAIL**、pglite **56 / 0** | **0** |
| `npm run typecheck` | 三个包全过 | **0** |
| R1 / R2 变异 | 24 FAIL / 8 FAIL（预期红） | 1 |

复现：

```bash
npm run verify:redact                                  # 只要 node，不需要数据库
VERIFY_DATABASE_URL='postgres://…/verifydb' npm run verify:db
```
