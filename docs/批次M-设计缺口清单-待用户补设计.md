...
> **没有它的位置与形态**。
> **用户 2026-09-24 已拍板**：浏览器工作区做成 **独立第四列**（原话选项「独立第四列」）。
> 在此之前 `browser/BrowserPanel.tsx` 的挂载方式保持原样不动、只重构它周围（M0 阶段已按此执行）。
> 第四列的**具体宽度/分隔/与三列的关系**仍待设计（见 §2）。
>
...
|---|---|---|---|---|
| 1 | **浏览器工作区**（整块） | `browser/BrowserPanel.tsx`（411 行）：顶栏活页 tab（+ 关闭 / 运行中圆点 / 休眠徽标）+ URL 栏 + 舞台（多智能体多页叠放）+ 自动休眠开关；由 `App.tsx:3357` 挂在 `div.browserLayer`（`:3310`）里 | ❌ 完全没有 | **位置已定：独立第四列**（用户 2026-09-24 拍板）。仍需你定：**列宽**、是否可拖拽（设计基准已有 `.splitter` 的交互可复用）、列头（活页 tab 条）形态、四列同时展开时各列的最小宽度 |
...
1. **浏览器工作区 = 独立第四列**（缺口 #1，**你已拍板，2026-09-24**）
   现在：`main.middle` 里绝对定位铺满、盖在 `.chat` 之上，`opacity` 切换（全屏/后台/嵌入三态），
   **元素永不卸载**（驾驶坐标依赖它）。改成第四列要动**挂载位置** —— 也就是说
   M0 冒烟网钉住的祖先链 golden 会在这一片**有意变更**（那时用 `--update-golden` 显式改，
   绝不自动重写），而**节点身份与"不许卸载"的规矩照旧不变**。
   仍需你定：列宽（默认值 + 可拖范围）、四列的最小宽度、列头 tab 条形态。

2. **状态层挂在哪**（缺口 #3/#4/#5/#6，**你已拍板：一起拍**）
   「谁在等谁」「待确认黄条」「任务卡」「头像状态圈」是同一类东西：**AI 运行时的状态可视化**。
   设计基准完全没有这一层。规格书已单独成文：
   **`docs/批次M-设计规格书-运行时状态层四件套.md`**（每件的触发条件 / 状态机 / 现有 DOM 与类名 /
   **必须保留的可辨识度** / 需要你定的项）。
