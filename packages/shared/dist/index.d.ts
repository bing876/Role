...
export interface WorkbenchBridge {
     ...
     */
    loopGoneChoice: (targetWebContentsId: number, choice: 'restart' | 'giveup') => Promise<TaskState>;
     ...
     */
    agentStart: (goal: string, apiBase: string, token: string, targetWebContentsId?: number, opts?: {
        agentId?: number | null;
        loopId?: string;
    }) => Promise<TaskState>;
     ...
     */
    browserThrottle: (webContentsId: number, throttle: boolean) => Promise<{
        ok: boolean;
        error?: string;
    }>;
    /**
     ...
     */
    browserWatchStart: (webContentsId: number) => Promise<{
        ok: boolean;
        error?: string;
    }>;
    /** 停掉这张页的 DOM 监听(幂等)。 */
    browserWatchStop: (webContentsId: number) => Promise<{
        ok: boolean;
    }>;
     ...
     */
    downloadDoc: (taskId: number, apiBase: string) => Promise<{
        saved: boolean;
        path?: string;
        canceled?: boolean;
        error?: string;
     ...
     */
    syncSession: (apiBase: string, token: string) => Promise<{
        ok: boolean;
        hasToken: boolean;
    }>;
    /**
     ...
     */
    syncProjects: (projectIds: number[]) => Promise<{
        ok: boolean;
        count?: number;
        error?: string;
    }>;
...
/**
 * 引导表填出来的人设。字段名就是表里的行标题，别改名。
 *
 * G2（2026-09-25）补两栏：
 *   - `antiJobs`（不干什么）：每个智能体的**标准栏**（规格 G2-②「每个 agent 增『不干什么』标准栏」）。
 *   - `description`（整份人设）：规格 G2-②「描述=整份人设」—— 这一栏就是完整人设正文
 *     （路由燃料 + 审批边界），不是被截断的摘要。
 * 两栏都是**可选**：存量人设（老数据）没有也不炸，新建/编辑时支持填。
 */
export interface AgentPersona {
    ...
    /** 干什么 */
    duty: string;
    /** 不干什么（anti-jobs）—— G2 标准栏 */
    antiJobs?: string;
    /** 整份人设描述（路由燃料 + 审批边界）—— G2：描述=整份人设 */
    description?: string;
}
...
/** 一次委派的状态机 */
export type DelegationStatus = 'pending' | 'running' | 'done' | 'failed' | 'timeout' | 'need_user' | 'rejected';
 ...
 */
export declare const SETTINGS_RANGE: {
    /**
     * 子阶段 A：上限从 8 放宽到 20 —— 默认值 20 必须落在合法区间里，
     * 否则「夹到区间」这一步会把默认值本身改回 8。
     */
    readonly maxConcurrentAgentTasks: {
        readonly min: 1;
        readonly max: 20;
    };
    readonly maxBrowserInstances: {
        readonly min: 1;
        readonly max: 20;
    };
    readonly resourceGuardEnabled: {
        readonly min: 0;
        readonly max: 1;
    };
    readonly resourceSampleMs: {
        readonly min: 1000;
        readonly max: 60000;
    };
    readonly resourceMemHealthMB: {
        readonly min: 256;
        readonly max: 65536;
    };
    readonly resourceMemWarnMB: {
        readonly min: 512;
        readonly max: 131072;
    };
    readonly resourceCpuHealthPct: {
        readonly min: 1;
        readonly max: 100;
    };
    readonly resourceCpuWarnPct: {
        readonly min: 2;
        readonly max: 100;
    };
    readonly resourceSysMemGuard: {
        readonly min: 0;
        readonly max: 1;
    };
    readonly resourceSysMemFloorMB: {
        readonly min: 128;
        readonly max: 32768;
    };
};
 ...
 */
export declare const DEFAULT_SETTINGS: WorkbenchSettings;
