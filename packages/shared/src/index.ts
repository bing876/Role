...
export interface WorkbenchBridge {
   ...
   */
  browserViewCreate: (req: { tabKey: number; projectId: number | null; url: string }) => Promise<{ wcId: number }>;
  /**
   * ADR-0002 · 页的**几何与可见性**（viewport 坐标 = 窗口内容区坐标）。
   * `visible: false` = 藏起来（隐藏≠卸载：视图仍附着、页面照跑）；尺寸/位置只在有值时生效。
   * 拖拽/滚动期间高频调用，主进程 rAF 合并、取最新。
   */
  browserViewRect: (req: { tabKey: number; rect: { x: number; y: number; width: number; height: number }; visible: boolean }) => void;
