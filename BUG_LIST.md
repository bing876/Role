...
## 复查批次（2026-09-22 · 真跑验证）

> 来源：`docs/系统健康度复查报告-20260922.md`。R = 复查新发现（均已真跑或代码实锤）。
> 本批次动作：零代码改动，修不修等用户拍板（见复查报告 §9）。

### 🔴 阻塞（新）

- **R1 聊天搜敏感信息直传第三方**：chat 路径 `web_search` 无敏感闸（只有 loop 有），卡号类查询原文发 Tavily。`search/chatLoop.ts`。修：搜前复用敏感正则拦截/`ask` + 日志脱敏。
- **R2 敏感输入值明文落库**：`agent.ts label()` 把 `type.text` 原文写进步摘要 → `/task/step` → `tasks.payload` 明文。修：摘要只记掩码/长度 + 服务端入库清洗。
- **R3 暂停吞回执→继续必重执行**：`pauseOut` 丢弃已算好的回执，resume 后服务端重发同一动作（S7 实测点两次「提交」）。修：暂停前刷回执 + 服务端改「回执缺失」话术 + 先读页核验。

### 🟠 高

- **R4 发车过触发（P0-1 反转）**：`shouldEnterTaskMode` 的 `length>=15` + 查/搜/看关键词把几乎所有聊天发车；loop 无 `web_search`，简单问题被开浏览器。修：收紧规则，纯提问走聊天。
- **#6 `done.clear()`**：仍开（`routes/memories.ts:369`）。修：按 key 淘汰最旧。
- **怀疑4 hijack 后 `dbErr`**：仍开（两分支，外层 catch `:660`）。修：hijack 标志位，之后只写 SSE。
- **R5 P0-3 搜索兜底漏申报**：机制仍在（S6-A 实锤静默改道百度）。修：兜底前敏感判定，敏感宁可 `ask`。
- **R6 `/next` 传输出错永久杀循环**：无重试退避，单次抖动 → `brain_failed` → `stopLoop`。修：传输错重试 3 次，不杀循环。
- **R7 `pageState` 跨租户串扰**：`pageStateOf` 不校验 `userId`，`clearPageState` 零调用。修：归属校验 + 关页清理。
- **R8 模型挂死 90 秒零反馈 + `failed` 终态**：`toolLoop.ts:930`。修：SSE 进度注记 + 超时重试 + 改 `waiting`。
- **R9 无 LLM 费用熔断**：限流只在短信口；`chatLoop` 无轮数上限（桩缺陷曾打出 700-call 自旋）。修：`MAX_ROUNDS` + 按用户/循环配额。
- **R10 敏感等待永不自动恢复**：`sensitiveHold` 从不被调用，watcher 只在求助卡路径。修：兑现钩契约 + 怀疑2 防护。

### 🟡 中（仍开/新）

- **P1-2/发现2**：`LoopToolResult` 无 `outcome` 区分（S2 实锤 click 落地判失败）；**S2** 同根因。
- **P1-3** 灰字丢任务、**P1-4** catch 无兜底（窗口已收窄）、**P1-6** 毫秒窗（建议接受并落盘）。
- **#9** 不中止（by design）、**#10** `tasks` Map 无清理、**#11** CORS 全回显（影响有限，修 allowlist）。
- **P1-5** 死代码、**怀疑5** 关页不停循环（升中：wcId 复用串 lane）、**dbErr 原文透传**。

### 本批已销/澄清

- **#7** 已修好（GROUP BY 对齐）；**怀疑3** 误报（索引存在）；**怀疑2** 暂不可达（修 R10 时顺手防）。
- **P2-5/P2-6** 自然消解（发车已不走本地正则，`detectBrowseIntent` 成死代码待清）；**P2-4** 已修（prompt 规则 0）；**P2-7** 已修（registry + 181 断言全绿）。
