...
class Cdp:
    ...
    def _connect(self, tries=3):
        ...
        for k in range(tries):
            try:
                ...
                last = e
                # ★ 关键：只要握手失败带 "No such target id"，说明页面换过（reload / 重开），
                #   缓存的目标 id 已经死了 —— 必须丢掉缓存重新 page() 找一次，
                #   否则后面每次重试都在拿同一个死 id 撞 500，一直撞到超时。
                if 'No such target id' in str(e):
                    self._t = None
                try:
                    self.ws.close()
    ...
    def send(self, method, **params):
        """一次调用；连接被对面关掉时换新连接重做（最多 4 次）。

        次数从 3 提到 4：页面刚 reload 的那一瞬间，第一次重连也常常还是会
        拿到旧目标（CDP 的 /json/list 有一小段缓存），需要多给它一次机会。
        """
        last = None
        for k in range(4):
            try:
        ...
        raise RuntimeError('CDP 调用 %s 连续 4 次掉线：%s' % (method, last))
    ...
    def type_text(self, text, sel=None, replace=True):
        ...
        time.sleep(0.2)
        # ★ Input.insertText 只在**浏览器层**把字塞进 input，不会派发 DOM 的 `input` 事件。
        #   React 的受控组件只认 `input` 事件 —— 不补这一下，`useState` 里还是空串，
        #   于是「建」按钮一直是 disabled，点了也毫无反应（表现为「建项目静默失败」）。
        #   这里手工派发一次，并让 React 的 value setter 走原生路径，状态才会真的更新。
        self.js(
            "(function(){const el=document.activeElement;"
            "if(!el||(el.tagName!=='INPUT'&&el.tagName!=='TEXTAREA'))return 'no-input';"
            "const d=Object.getOwnPropertyDescriptor("
            "el.tagName==='INPUT'?window.HTMLInputElement.prototype:window.HTMLTextAreaElement.prototype,'value');"
            "el.dispatchEvent(new Event('input',{bubbles:true}));"
            "return String(d&&d.get?d.get.call(el):el.value);})()")
        time.sleep(0.3)
