...
export interface BrowserTabView {
  ...
  title: string;
  /**
   * 休眠深度（第 24 步起）。`undefined` = 清醒。
   *
   * 两级是有意分开的，因为它们省的资源不同：
   *   - `'shallow'`：**省 CPU**。`<webview>` 还挂着、页面还活着，只是停止被驾驶、
   *     并把后台节流开到最大（视频不再后台解码）。恢复是瞬时的，没有任何白屏。
   *   - `'deep'`：**省内存**。`<webview>` 真的被卸载，只留「地址 + 滚动位置 + 表单草稿」，
   *     唤醒时重新加载并还原。这才对应「不占内存」。
   *
   * ★ 正在被驾驶的页（drivingIds）**永远不会是休眠态** —— 收起来等于把任务弄断。
   */
  sleep?: 'shallow' | 'deep';
}
